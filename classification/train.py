...
of the existing class-weighting fix and DataParallel/AMP acceleration:
  ...
  5. Architecture parameterization (this revision) -- build_model() was
     hardcoded to EfficientNet-B3 only, and the checkpoint filename was
     hardcoded to "efficientnet_b3_best.pt" in two places (the save line
     and the early-stopping log message), blocking the ConvNeXt-Tiny/
     ResNet34 runs. Both are now driven by a new --model-name CLI flag:
     architecture construction moves to classification/model_registry.py
     (imported below, not reimplemented here), which handles the two known
     head-layout cases -- EfficientNet-B3/ConvNeXt-Tiny's `.classifier[-1]`
     vs ResNet34's plain `.fc` -- in one place; the checkpoint filename is
     derived from the same flag via checkpoint_filename(). Device
     placement and DataParallel wrapping stay here, since neither is
     architecture-specific. --model-name uses the same strings
     push_checkpoint.py's own --model-name expects, so no translation
     layer is needed between the two scripts. Nothing about the training
     loop, loss weighting, label smoothing, scheduler, or early stopping
     changed -- per the locked decision that fine-tuning stays identical
...
from classification.model_registry import (
    build_model as build_classification_model,
    checkpoint_filename,
    MODEL_REGISTRY,
)

# DATASET SWITCH (2026-09-18): previously pointed at the crop-extraction
# dataset (pre-axial-laterality-fix manifest). That dataset has been
# detached from this project's sessions and is no longer relied on.
# Now points at axial-laterality-fix-crops -- the post-migration dataset,
# confirmed self-sufficient before this switch was made: all 41,333
# manifest rows resolve to an existing crop file under this dataset's own
# root (100% per condition, all 5 conditions checked directly, not
# assumed). Update if this ever moves.
MANIFEST_PATH = "/kaggle/input/datasets/elenoremadams/axial-laterality-fix-crops/manifest.csv"
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Architecture to train. Same strings push_checkpoint.py's --model-name expects.",
    )
    return parser.parse_args()


def build_model(device, model_name):
    """Architecture construction (weights + head swap) lives in
    classification/model_registry.py -- imported above as
    build_classification_model, not reimplemented here. This function
    keeps only what's NOT architecture-specific: device placement and
    DataParallel wrapping."""
    model = build_classification_model(model_name)
...
def main():
    ...
    print(f"[train] device={device}  model_name={args.model_name}")
    os.makedirs(CHECKPOINT_DIR, exist_ok=True)
    ckpt_filename = checkpoint_filename(args.model_name)
        ...
        if val_loss < best_val_loss:
            ...
            if epochs_without_improvement >= EARLY_STOPPING_PATIENCE:
                print(f"[train] early stopping -- no val_loss improvement for "
                      ...
                      f"{os.path.join(CHECKPOINT_DIR, ckpt_filename)}.")
                break

    print(f"[train] training complete. model_name={args.model_name}  best_val_loss={best_val_loss:.4f}")
