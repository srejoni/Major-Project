...
a "was_duplicate_resolved" flag through each crop entry -- True whenever
more than one series_id contributed a candidate detection for that
(series, level) and the higher-confidence one was kept. This was
previously discarded before reaching this file, meaning
status="duplicate_resolved" (documented and promised in
pdscd_output_format.md §5) could never actually appear in real output. It
is now set correctly below instead of defaulting every successful
prediction to status="ok".

NAMING FIX (this pass): the crop-dict key read here was previously
"had_duplicate", while crop_extraction.py's training-time manifest column
for the identical concept was already "was_duplicate_resolved". Renamed
the read here to match -- one name for one concept across the manifest
column, the inference crop dict, and the schema's status value, instead of
two names for the same thing that happen not to collide today.
...
def main():
    ...
    parser.add_argument("--schema-path", default="/kaggle/working/PDSCD/classification/pdscd_output_schema.json")
