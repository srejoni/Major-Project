...
confirmed convention. There is no external convention to check this
against either: the RSNA competition's own submission format has no
"missing prediction" concept (every row must carry a probability; ground
truth NaNs are excluded from scoring, not weighted), so this decision is
internal to this schema and needs sign-off from whoever owns the format
doc before shipping.
...
MISSING_DETECTION_WEIGHT_DEFAULT = 1  # see "OPEN SPEC QUESTION" above -- unconfirmed
