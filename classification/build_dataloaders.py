...
def _resolve_crop_root(sample_path: str) -> str:
    ...
    ORDER FIX (earlier pass): this session's own /kaggle/working output is
    checked BEFORE the read-only cross-account input Dataset. Previously
    the input Dataset was checked first and silently won any time its
    directory structure happened to match the manifest's tail path -- which
    is every time, since the crop naming convention (series_slug +
    level_slug + .npy) hasn't changed. That meant a from-scratch rerun's
    freshly-generated crops in /kaggle/working could be silently ignored in
    favor of an older, possibly pre-fix dataset, with no error or warning --
    the function would report success either way. The /kaggle/working
    candidates stay checked first for exactly this reason.

    DATASET SWITCH (2026-09-18): the cross-account input Dataset candidate
    previously pointed at elenoremadams/crop-extraction/classification_data
    (an earlier version of this docstring referred to this as
    "srejoni/crop-extraction" -- stale; the account in use is
    elenoremadams, corrected here). That dataset predates the axial
    laterality fix and has now been detached from this project's sessions.
    It has been replaced with elenoremadams/axial-laterality-fix-crops --
    note this dataset's root has no "classification_data" subfolder (crops/
    and manifest.csv sit directly at its root), so the candidate string
    itself does not have that suffix, unlike the old entry it replaces.
    Confirmed self-sufficient before this switch: all 41,333 manifest rows
    resolve to an existing file under this candidate root, 100% across all
    5 conditions, checked directly rather than assumed.

    No fallback to the old crop-extraction dataset is kept, deliberately.
    It is no longer attached to any session, so keeping it in this list
    would only ever silently fail past it to the next candidate anyway; if
    every real candidate below is ever wrong or unmounted, the intended
    failure is a loud FileNotFoundError from this function, not a silent
    resolve to a stale pre-fix crop set.
    """
    candidates = [
    "/kaggle/working/classification_data",
    "/kaggle/working/PDSCD/classification_data",
    "/kaggle/input/datasets/elenoremadams/axial-laterality-fix-crops",
]
