...
Loads pre-cropped classification patches saved by crop_extraction.py. Every
crop is a genuine (H, W, 3) array -- sagittal replicated to 3 identical
channels, axial as 3 real neighboring slices. This class only transposes
and normalizes; it does NOT replicate channels itself.
...
Normalization is delegated to classification/normalization.py -- the single
shared function used by both this class and assemble_study_output.py at
inference time, so the two can never drift apart again.
"""
import random
