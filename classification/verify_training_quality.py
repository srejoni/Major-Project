...
FIX (architecture parameterization): load_model() previously hardcoded
torchvision.models.efficientnet_b3 -- fine while EfficientNet-B3 was the
only trained checkpoint, but silently wrong for any other architecture.
Generalized to take a required --model-name flag.

FIX (stop duplicating architecture construction, this pass): the
generalized version of load_model() briefly had its own MODEL_BUILDERS
dict + manual "read in_features off classifier[-1], swap in a 3-class
Linear" logic here -- a second, independent copy of exactly what
classification/model_registry.py::build_model() now centralizes as the
single source of truth (including resnet34's genuinely different `.fc`
head, which this file's old hand-rolled version didn't even attempt).
Now imports build_model from there instead. Verified this is safe before
switching, not assumed: ran model_registry.py's own smoke test
(`python -m classification.model_registry`) and confirmed all 3
architectures -- including resnet34's `.fc` branch -- build correctly and
produce (N, 3) output with every param trainable.

Practical effect of this refactor: resnet34 support here is now free.
Once a resnet34 checkpoint exists, `--model-name resnet34` just works --
no code change in this file, because the .fc-vs-.classifier[-1] branching
lives in model_registry.py, not here.

--checkpoint has no hardcoded default (there is no single "right" path
once more than one architecture exists) -- both --checkpoint and
--model-name are required, so a mismatched pair is a conscious choice,
not an accident.
"""
import sys
import argparse
...
from classification.model_registry import build_model as build_classification_model, MODEL_REGISTRY
...
def load_model(checkpoint_path, model_name, device):
    """Architecture construction delegated entirely to model_registry.py --
    see module docstring for why. pretrained=False: we're about to
    overwrite every weight with the checkpoint's state_dict anyway, so
    there's no reason to download ImageNet weights first (same reasoning
    model_registry.py's own offline smoke test uses)."""
    if model_name not in MODEL_REGISTRY:
        raise ValueError(
            f"Unknown --model-name '{model_name}'. Choose from {sorted(MODEL_REGISTRY)} "
            f"(classification/model_registry.py's MODEL_REGISTRY)."
        )
...
def print_report(by_condition, model_name):
    print("\n" + "=" * 70)
    print(f"PER-CONDITION BREAKDOWN  (model={model_name})")
    print("=" * 70)
...
def main():
    ...
    parser.add_argument("--checkpoint", required=True,
                         help="Path to the .pt checkpoint to verify. No default -- "
                              "there is no single 'right' path once more than one "
                              "architecture exists, so pass it explicitly.")
    parser.add_argument("--model-name", required=True, choices=sorted(MODEL_REGISTRY),
                         help="Which architecture this checkpoint belongs to. Must match "
                              "the architecture it was actually trained as -- loading a "
                              "convnext_tiny checkpoint as efficientnet_b3 (or vice versa) "
                              "fails on state_dict key mismatch, by design.")
    ...
    print(f"[verify] model_name={args.model_name}")
    ...
    print_report(by_condition, args.model_name)
