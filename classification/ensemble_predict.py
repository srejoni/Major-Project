...
BLOCKER, NOT WORKED AROUND (updated): pdscd_output_schema.json requires
model_versions and model_votes to contain all three of efficientnet_b3,
convnext_tiny, resnet34 (additionalProperties: false). Now that
efficientnet_b3 AND convnext_tiny have trained checkpoints, resnet34 is
the only architecture still missing from MODEL_REGISTRY below. This file
does NOT fabricate placeholder votes for the missing one to force a
schema pass -- copying an existing result into the resnet34 slot would
misrepresent partial-ensemble output as three-way agreement, exactly the
distinction model_agreement exists to protect. Add resnet34 to
MODEL_REGISTRY once a real checkpoint exists; no other code change needed.
"""
import os
import sys
...
MODEL_REGISTRY = {
    ...
    # "resnet34": {"checkpoint": f"{REPO_ROOT}/classification/weights/resnet34_best.pt"},  # NOT YET TRAINED
}
