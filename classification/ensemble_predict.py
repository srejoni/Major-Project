...
FIX (checkpoint path): CHECKPOINT_REGISTRY previously pointed at
...
FIX (ConvNeXt-Tiny support): added a convnext_tiny CHECKPOINT_REGISTRY entry,
...
CHECKPOINT_REGISTRY below -- no new loader function needed, unlike before.
...
registry entry below was left commented out with a stale "NOT YET
...
that drift: it only iterates whatever's in the registry dict, so a
correctly commented-out entry and an incorrectly-still-commented-out
entry look identical to it -- 2 models loaded, zero errors, no signal
anything was wrong until a downstream caller's separate 3-model guard
caught it. Now uncommented. All 3 architectures load.

FIX (name collision, this pass): this file's dict was also called
MODEL_REGISTRY -- same name as classification/model_registry.py's
MODEL_REGISTRY, a different dict with a different shape (arch name ->
checkpoint path here, vs. arch name -> {ctor, weights, head_attr,
head_is_sequential} there). Same name, unrelated purpose, defined in two
files that both get imported into this one's neighborhood -- exactly the
kind of seam a stale-entry bug like the one above is easy to miss in on
a skim. Renamed this file's dict to CHECKPOINT_REGISTRY; no change to
model_registry.py or its MODEL_REGISTRY, which keeps its name.
"""
import os
import sys
...
# no new function. Named CHECKPOINT_REGISTRY, not MODEL_REGISTRY, to stay
# distinct from classification/model_registry.py's MODEL_REGISTRY -- same
# word, different dict, different file; the collision is what let
# resnet34's entry sit stale here without standing out on a skim.
CHECKPOINT_REGISTRY = {
    "efficientnet_b3": {
...
def load_models(device):
    for name, spec in CHECKPOINT_REGISTRY.items():
        if not os.path.exists(spec["checkpoint"]):
            ...
            raise FileNotFoundError(
                ...
                f"then update CHECKPOINT_REGISTRY['{name}']['checkpoint'] to match."
            )
