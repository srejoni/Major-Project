...
def load_completed_study_ids():
    ...
    SCHEMA GUARD (this pass): before trusting an existing manifest as
    resumable state, confirm its header actually matches MANIFEST_FIELDS.
    A manifest left over from before was_duplicate_resolved was added
    would silently get new 7-column rows appended under its old 6-column
    header -- pandas may then either throw a confusing tokenizing error,
    or in some configurations misalign columns rather than fail loudly.
    Failing here, once, with a clear message, is cheaper than debugging a
    quietly corrupted manifest later or relying on remembering to delete
    it by hand before every rerun.
    """
...
def _canonicalize_level_string(s):
    ...
    actual model: model.names = {0: 'L1-L2', 1: 'L2-L3', 2: 'L3-L4',
    3: 'L4-L5', 4: 'L5-S1'} -- positionally identical to LEVELS, only the
    delimiter differs (YOLO/data.yaml class names typically can't contain
    '/'). Normalizing both to the same delimiter before comparing lets the
    check verify what actually matters -- index order -- without being
    defeated by a cosmetic formatting difference."""
...
def validate_yolo_class_mapping(model):
    ...
    _canonicalize_level_string). This was unverified since
    pdscd_progress_update_v2.md flagged it as the highest-risk open item;
    a real mismatch here would mislabel every crop by level, permanently,
    with no error anywhere. A delimiter difference alone is not that bug --
    only a genuine positional/order mismatch should raise.
    """
