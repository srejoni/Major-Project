...
Fixes applied after code review (this pass):
...
- validate_yolo_class_mapping() -- confirms best.pt's own class
  ...
  produced. This was the single highest-risk unverified item flagged in
  review: detect_best_per_level() uses int(box.cls.item()) directly as an
  index into LEVELS with nothing checking that assumption. A silent
  mismatch here would mislabel every crop by level, permanently, with no
  error anywhere -- this check turns that into a loud failure at startup.
"""
