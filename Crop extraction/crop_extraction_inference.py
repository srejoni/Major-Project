...
entry now carries "was_duplicate_resolved": True whenever more than one
series_id contributed candidate detections for that (series_description,
level), regardless of which one ultimately won on confidence.

NAMING FIX (this pass): this field was previously called "had_duplicate"
here, while crop_extraction.py's manifest column for the same concept was
already "was_duplicate_resolved". Two names for one concept is exactly
the class of drift this project has been burned by before (normalization,
dataloader signature) -- even though the two never collided at runtime
(different pipelines, different consumers), there's no reason to leave a
second name for the same thing lying around. Renamed to
"was_duplicate_resolved" here and at the assemble_study_output.py call
site so the manifest column, the inference crop dict key, and the schema
concept they all describe use one name end to end.
"""
import sys
