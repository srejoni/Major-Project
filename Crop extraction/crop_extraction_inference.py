...
FIX (previous pass -- Sagittal T1 laterality): crop_extraction.py's module
docstring has the full diagnostic chain. Short version: left/right
foraminal narrowing were confirmed to always receive an identical crop
(both conditions map to "Sagittal T1" in SERIES_BY_CONDITION, and the
merge was keyed by (series_desc, level) only). train_label_coordinates.csv
confirms left and right genuinely differ by slice in ~99.6% of
foraminal-narrowing cases, and a geometric proxy (DICOM
ImagePositionPatient projected onto the imaging plane's normal vector) was
verified at 100% sign consistency across 125 real left/right pairs to
reliably tell which slice is which side.

FIX (THIS pass -- Axial T2 laterality): the same shared-crop bug existed
for left/right subarticular stenosis (both map to "Axial T2"), and is now
fixed the same way in spirit but with a different proxy -- axial slices
stack along the superior-inferior axis, so left/right usually live WITHIN
one image rather than on different slices; see
crop_extraction.py's module docstring for the full dual-mode
(relative-when-paired / calibrated-absolute-when-singleton) diagnostic
chain and validated accuracy figures.

This file now calls crop_extraction.merge_series_detections_by_side() for
Sagittal T1 and crop_extraction.merge_series_detections_axial_dual() for
Axial T2 -- both in SIDE_SPLIT_SERIES, dispatched via AXIAL_DUAL_MODE_SERIES
-- instead of the single-crop merge_series_detections(), and picks the
correct side per condition via crop_extraction.condition_side(). Sagittal
T2/STIR (canal stenosis, non-lateral) is unchanged -- one shared crop per
level, as before.
...
from classification.crop_extraction import (
    ...
    AXIAL_DUAL_MODE_SERIES, build_stacked_crop, merge_series_detections,
    merge_series_detections_by_side, merge_series_detections_axial_dual,
    condition_side,
)
...
def extract_study_crops(model, study_id, series_df, data_root):
    ...
    conditions now resolve to genuinely different crops via the geometric
    side-split proxy (cross-slice position for Sagittal T1, dual-mode
    relative/calibrated-absolute for Axial T2), matching training exactly
    (same shared functions, same selection logic). For Sagittal T2/STIR
    (non-lateral), behavior is unchanged -- one shared crop per level.
